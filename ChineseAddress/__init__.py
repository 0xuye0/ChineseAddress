...
#***************** OPTIONAL CONFIG ***************************************************
PARENT_LABEL  = 'ChineseAddressString'               # the XML tag for each labeled string
GROUP_LABEL   = 'ChineseAddressCollection'                  # the XML tag for a group of strings
#NULL_LABEL    = 'xyNull'                        # the null XML tag
MODEL_FILE    = 'chnaddr.crfsuite'   # filename for the crfsuite settings file
#************************************************************************************
...
def tokenize(raw_string):
    ...
    #re_tokens = re.compile(' .*? ')# re.compile( [REGEX HERE], re.VERBOSE | re.UNICODE)
...
def tokenFeatures(token) :
    ...
    policy_token, pre_token = get_policy(token)
...
def casing(token) :
    if token.isupper() :
        ...
        return False
def get_policy(token) :
    for i in range(len(name)):
        if token.find(name[i]) != -1:
            return name[i], token.replace(name[i],"")
    return "NULL",token
